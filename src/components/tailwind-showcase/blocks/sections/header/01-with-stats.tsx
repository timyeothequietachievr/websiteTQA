    ...
    <div className="relative isolate overflow-hidden bg-paper py-24 sm:py-32">
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-2xl lg:mx-0">
          <h2 className="text-5xl font-display font-semibold tracking-tight text-charcoal sm:text-7xl">
            Work with us
          </h2>
          <p className="mt-8 text-lg font-medium text-pretty text-warm-700 sm:text-xl/8">
            Anim aute id magna aliqua ad ad non deserunt sunt. Qui irure qui lorem cupidatat commodo. Elit sunt amet
            fugiat veniam occaecat fugiat.
          </p>
        </div>
        <div className="mx-auto mt-10 max-w-2xl lg:mx-0 lg:max-w-none">
          <div className="grid grid-cols-1 gap-x-8 gap-y-6 text-base/7 font-semibold text-charcoal sm:grid-cols-2 md:flex lg:gap-x-10">
            {links.map((link) => (
          ...
          <dl className="mt-16 grid grid-cols-1 gap-8 sm:mt-20 sm:grid-cols-2 lg:grid-cols-4">
            {stats.map((stat) => (
              <div key={stat.name} className="flex flex-col-reverse gap-1">
                <dt className="text-base/7 text-warm-700">{stat.name}</dt>
                <dd className="text-4xl font-display font-semibold tracking-tight text-charcoal">{stat.value}</dd>
              </div>
            ))}
          </dl>
        </div>
