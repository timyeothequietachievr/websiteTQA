      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-6 sm:mt-20 lg:mx-0 lg:max-w-none lg:grid-cols-3 lg:gap-8">
          {cards.map((card) => (
            <div
              key={card.name}
              className="flex gap-x-4 rounded-xl bg-paper/30 p-6 ring-1 ring-charcoal/5 backdrop-blur-sm"
            >
              <card.icon aria-hidden="true" className="h-7 w-5 flex-none text-ember" />
              <div className="text-base/7">
                <h3 className="font-semibold text-charcoal">{card.name}</h3>
                <p className="mt-2 text-warm-700">{card.description}</p>
              </div>
            </div>
          ))}
        </div>
