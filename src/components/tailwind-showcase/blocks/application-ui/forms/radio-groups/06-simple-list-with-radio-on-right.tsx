...
export default function TailwindBlock() {
  return (
    <fieldset>
      <legend className="text-sm/6 font-semibold text-charcoal">Select a side</legend>
      <div className="mt-4 divide-y divide-gray-200 border-t border-b border-paper-200">
        {sides.map((side, sideIdx) => (
          <div key={sideIdx} className="relative flex items-start py-4">
            <div className="min-w-0 flex-1 text-sm/6">
              <label htmlFor={`side-${side.id}`} className="font-medium text-charcoal select-none">
                {side.name}
              </label>
            </div>
