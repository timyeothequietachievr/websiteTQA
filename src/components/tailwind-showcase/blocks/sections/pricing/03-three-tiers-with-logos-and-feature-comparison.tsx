  ...
  return (
    <div className="bg-paper py-24 sm:py-32">
      <div className="mx-auto max-w-4xl px-6 max-lg:text-center lg:max-w-7xl lg:px-8">
        <h1 className="text-5xl font-display font-semibold tracking-tight text-balance text-gray-950 sm:text-6xl lg:text-pretty">
          Pricing that grows with your team size
        </h1>
        <p className="mt-6 max-w-2xl text-lg font-medium text-pretty text-warm-500 max-lg:mx-auto sm:text-xl/8">
        ...
        <div className="relative mx-auto max-w-2xl px-6 lg:max-w-7xl lg:px-8">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-3">
            {tiers.map((tier) => (
              <div
                key={tier.name}
                className="-m-2 grid grid-cols-1 rounded-4xl bg-paper/2.5 shadow-[inset_0_0_2px_1px_#ffffff4d] ring-1 ring-black/5 max-lg:mx-auto max-lg:w-full max-lg:max-w-md"
              >
                <div className="grid grid-cols-1 rounded-4xl p-2 shadow-md shadow-black/5">
                  <div className="rounded-3xl bg-paper p-10 pb-9 shadow-2xl ring-1 ring-black/5">
                    <h2 className="text-sm font-semibold text-ember">
                      {tier.name} <span className="sr-only">plan</span>
                    </h2>
                    <p className="mt-2 text-sm/6 text-pretty text-warm-500">{tier.description}</p>
                    ...
                    <div className="mt-8">
                      <a
                        ...
                        className="inline-block rounded-md bg-ember px-3.5 py-2 text-center text-sm/6 font-semibold text-paper shadow-xs hover:bg-ember focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ember"
                      >
                        Start a free trial
                      </a>
                    </div>
                      ...
                      <ul className="mt-3 space-y-3">
                        {tier.highlights.map((highlight) => (
                          <li
                            ...
                            className="group flex items-start gap-4 text-sm/6 text-warm-500 data-disabled:text-warm-500"
                          >
              ...
              {tiers.map((tier) => (
                <td key={tier.name} className="px-0 pt-3 pb-0">
                  <a
                    ...
                    className="inline-block rounded-md bg-paper px-2.5 py-1.5 text-sm font-semibold text-charcoal shadow-xs inset-ring-1 inset-ring-gray-300 hover:bg-paper-soft"
                  >
                    Get started
                  </a>
                </td>
            ...
            <tbody key={section.name} className="group">
              <tr>
                <th scope="colgroup" colSpan={4} className="px-0 pt-10 pb-0 group-first-of-type:pt-5">
                  <div className="-mx-4 rounded-lg bg-paper-soft px-4 py-3 text-sm/6 font-semibold text-gray-950">
                    {section.name}
                  </div>
                </th>
              </tr>
              {section.features.map((feature) => (
                <tr key={feature.name} className="border-b border-gray-100 last:border-none">
                  <th scope="row" className="px-0 py-4 text-sm/6 font-normal text-warm-500">
                    {feature.name}
                  </th>
                  {tiers.map((tier) => (
        ...
        <TabGroup className="sm:hidden">
          <TabList className="flex">
            {tiers.map((tier) => (
              <Tab
                key={tier.name}
                className="w-1/3 border-b border-gray-100 py-4 text-base/8 font-medium text-ember not-focus-visible:focus:outline-none data-selected:border-indigo-600"
              >
                {tier.name}
              </Tab>
            ))}
          ...
          <TabPanels as={Fragment}>
            {tiers.map((tier) => (
              <TabPanel key={tier.name} className="focus:outline-none">
                <a
                  href={tier.href}
                  className="mt-8 block rounded-md bg-paper px-3.5 py-2.5 text-center text-sm font-semibold text-charcoal shadow-xs inset-ring ring-gray-300 hover:bg-paper-soft"
                >
                  Get started
                </a>
                {sections.map((section) => (
                  <Fragment key={section.name}>
                    <div className="-mx-6 mt-10 rounded-lg bg-paper-soft px-6 py-3 text-sm/6 font-semibold text-gray-950 group-first-of-type:mt-5">
                      {section.name}
                    </div>
                    <dl>
                      {section.features.map((feature) => (
                        ...
                        >
                          <dt className="text-sm/6 font-normal text-warm-500">{feature.name}</dt>
