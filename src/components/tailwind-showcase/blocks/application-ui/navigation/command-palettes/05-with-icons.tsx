  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 z-10 w-screen overflow-y-auto p-4 sm:p-6 md:p-20">
        <DialogPanel
          transition
          className="mx-auto max-w-2xl transform divide-y divide-gray-100 overflow-hidden rounded-xl bg-paper shadow-2xl outline-1 outline-black/5 transition-all data-closed:scale-95 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in"
        >
          <Combobox
            onChange={(item) => {
              if (item) {
          ...
          >
            ...
            {(query === '' || filteredProjects.length > 0) && (
              ...
              >
                <li className="p-2">
                  {query === '' && (
                    <h2 className="mt-4 mb-2 px-3 text-xs font-semibold text-warm-500">
                      Recent searches
                    </h2>
                  )}
                  <ul className="text-sm text-warm-700">
                    {(query === '' ? recent : filteredProjects).map((project) => (
                      <ComboboxOption
                        ...
                        key={project.id}
                        value={project}
                        className="group flex cursor-default items-center rounded-md px-3 py-2 select-none data-focus:bg-ember data-focus:text-paper data-focus:outline-hidden"
                      >
                        <FolderIcon
                    ...
                    <ul className="text-sm text-warm-700">
                      {quickActions.map((action) => (
                        ...
                        >
                          ...
                          <span className="ml-3 flex-none text-xs font-semibold text-warm-500 group-data-focus:text-indigo-100">
            ...
            {query !== '' && filteredProjects.length === 0 && (
              <div className="px-6 py-14 text-center sm:px-14">
                <FolderIcon className="mx-auto size-6 text-warm-500" aria-hidden="true" />
