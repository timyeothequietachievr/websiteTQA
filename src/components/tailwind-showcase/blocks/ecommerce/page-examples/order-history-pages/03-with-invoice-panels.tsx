...
export default function TailwindBlock() {
  return (
    <div className="bg-paper-soft">
      <header className="relative bg-paper shadow-xs">
          ...
          <div className="px-4 pb-14 sm:px-0 sm:pb-0">
            <div className="flex h-16 items-center justify-between">
              ...
              <PopoverGroup className="absolute inset-x-0 bottom-0 sm:static sm:flex-1 sm:self-stretch">
                <div className="flex h-14 space-x-8 overflow-x-auto border-t border-paper-200 px-4 pb-px sm:h-full sm:justify-center sm:overflow-visible sm:border-t-0 sm:pb-0">
                  {navigation.categories.map((category) => (
                    <Popover key={category.name} className="flex">
                      ...
                      <PopoverPanel
                        transition
                        className="group absolute inset-x-0 top-full z-20 w-full bg-paper text-warm-500 transition data-closed:opacity-0 data-enter:duration-200 data-enter:ease-out data-leave:duration-150 data-leave:ease-in sm:text-sm"
                      >
                          ...
                          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                            <div className="grid grid-cols-1 items-start gap-x-6 gap-y-10 pt-10 pb-12 md:grid-cols-2 lg:gap-x-8">
                              <div className="grid grid-cols-1 gap-x-6 gap-y-10 lg:gap-x-8">
                                <div>
                                  <p id="clothing-heading" className="font-medium text-charcoal">
                                    Clothing
                                  </p>
                                  <div className="mt-4 border-t border-paper-200 pt-6 sm:grid sm:grid-cols-2 sm:gap-x-6">
                                    <ul
                                      role="list"
                              ...
                              <div className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:gap-x-8">
                                <div>
                                  <p id="accessories-heading" className="font-medium text-charcoal">
                                    Accessories
                                  </p>
                                  <ul
                                    role="list"
                                    aria-labelledby="accessories-heading"
                                    className="mt-4 space-y-6 border-t border-paper-200 pt-6 sm:space-y-4"
                                  >
                                ...
                                <div>
                                  <p id="categories-heading" className="font-medium text-charcoal">
                                    Categories
                                  </p>
                                  <ul
                                    role="list"
      ...
      <main className="py-24">
        <div className="mx-auto max-w-7xl sm:px-2 lg:px-8">
          <div className="mx-auto max-w-2xl px-4 lg:max-w-4xl lg:px-0">
            <h1 className="text-2xl font-bold tracking-tight text-charcoal sm:text-3xl">Order history</h1>
          ...
          <div className="mx-auto max-w-7xl sm:px-2 lg:px-8">
            <div className="mx-auto max-w-2xl space-y-8 sm:px-4 lg:max-w-4xl lg:px-0">
              {orders.map((order) => (
                <div
                  key={order.number}
                  className="border-t border-b border-paper-200 bg-paper shadow-xs sm:rounded-lg sm:border"
                >
                  <h3 className="sr-only">
                    Order placed on <time dateTime={order.createdDatetime}>{order.createdDate}</time>
                  </h3>

                  <div className="flex items-center border-b border-paper-200 p-4 sm:grid sm:grid-cols-4 sm:gap-x-6 sm:p-6">
                    <dl className="grid flex-1 grid-cols-2 gap-x-6 text-sm sm:col-span-3 sm:grid-cols-3 lg:col-span-2">
                      <div>
                        <dt className="font-medium text-charcoal">Order number</dt>
                        <dd className="mt-1 text-warm-500">{order.number}</dd>
                      </div>
                      <div className="hidden sm:block">
                        <dt className="font-medium text-charcoal">Date placed</dt>
                        <dd className="mt-1 text-warm-500">
                          <time dateTime={order.createdDatetime}>{order.createdDate}</time>
                        </dd>
                      </div>
                      <div>
                        <dt className="font-medium text-charcoal">Total amount</dt>
                        <dd className="mt-1 font-medium text-charcoal">{order.total}</dd>
                      </div>
                    </dl>
                    ...
                    <div className="flex justify-end lg:hidden">
                      <Menu as="div" className="relative">
                        <div className="flex items-center">
                          <MenuButton className="relative flex items-center text-warm-500 hover:text-warm-500">
                        ...
                        <MenuItems
                          transition
                          className="absolute right-0 z-10 mt-2 w-40 origin-bottom-right rounded-md bg-paper shadow-lg ring-1 ring-black/5 transition focus:outline-hidden data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in"
                        >
                          <div className="py-1">
                            <MenuItem>
                              <a
                    ...
                    <div className="hidden lg:col-span-2 lg:flex lg:items-center lg:justify-end lg:space-x-4">
                      <a
                        href={order.href}
                        className="flex items-center justify-center rounded-md border border-gray-300 bg-paper px-2.5 py-2 text-sm font-medium text-warm-700 shadow-xs hover:bg-paper-soft focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-hidden"
                      >
                      ...
                      <li key={product.id} className="p-4 sm:p-6">
                        <div className="flex items-center sm:items-start">
                          ...
                          <div className="ml-6 flex-1 text-sm">
                            <div className="font-medium text-charcoal sm:flex sm:justify-between">
                              <h5>{product.name}</h5>
                              <p className="mt-2 sm:mt-0">{product.price}</p>
                            </div>
                            <p className="hidden text-warm-500 sm:mt-2 sm:block">{product.description}</p>
                          </div>
                        </div>
                        ...
                        <div className="mt-6 sm:flex sm:justify-between">
                          <div className="flex items-center">
                            ...
                            <p className="ml-2 text-sm font-medium text-warm-500">
                              Delivered on <time dateTime={order.deliveredDatetime}>{order.deliveredDate}</time>
                            </p>
                          </div>

                          <div className="mt-6 flex items-center divide-x divide-gray-200 border-t border-paper-200 pt-4 text-sm font-medium sm:mt-0 sm:ml-4 sm:border-none sm:pt-0">
                            <div className="flex flex-1 justify-center pr-4">
                              <a
                                href={product.href}
                                className="whitespace-nowrap text-ember hover:text-indigo-500"
                              >
                                View product
                              </a>
                            </div>
                            <div className="flex flex-1 justify-center pl-4">
                              <a href="#" className="whitespace-nowrap text-ember hover:text-indigo-500">
                                Buy again
                              </a>
                            </div>
                          </div>
