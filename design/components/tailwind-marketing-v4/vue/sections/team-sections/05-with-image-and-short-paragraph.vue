<template>
  <div class="bg-white py-24 md:py-32 lg:py-40 dark:bg-gray-900">
    <div class="mx-auto grid max-w-7xl grid-cols-1 gap-20 px-6 lg:px-8 xl:grid-cols-3">
      ...
      <ul role="list" class="mx-auto grid max-w-2xl grid-cols-1 gap-x-6 gap-y-20 sm:grid-cols-2 lg:mx-0 lg:max-w-none lg:gap-x-8 xl:col-span-2">
        <li v-for="person in people" :key="person.name">
          <img class="aspect-3/2 w-full rounded-2xl object-cover outline-1 -outline-offset-1 outline-black/5 dark:outline-white/10" :src="person.imageUrl" alt="" />
          <h3 class="mt-6 text-lg/8 font-semibold text-gray-900 dark:text-white">{{ person.name }}</h3>
          <p class="text-base/7 text-gray-600 dark:text-gray-400">{{ person.role }}</p>
          <p class="mt-4 text-base/7 text-gray-600 dark:text-gray-400">{{ person.bio }}</p>
