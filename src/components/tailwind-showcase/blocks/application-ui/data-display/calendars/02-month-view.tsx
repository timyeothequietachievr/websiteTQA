...
export default function TailwindBlock() {
  return (
    <div className="lg:flex lg:h-full lg:flex-col">
      <header className="flex items-center justify-between border-b border-paper-200 px-6 py-4 lg:flex-none">
        ...
        <div className="flex items-center">
          ...
          <Menu as="div" className="relative ml-6 md:hidden">
            <MenuButton className="-mx-2 flex items-center rounded-full border border-transparent p-2 text-warm-500 hover:text-warm-500">
      ...
      <div className="shadow-sm ring-1 ring-black/5 lg:flex lg:flex-auto lg:flex-col">
        <div className="grid grid-cols-7 gap-px border-b border-gray-300 bg-gray-200 text-center text-xs/6 font-semibold text-warm-700 lg:flex-none">
          <div className="flex justify-center bg-paper py-2">
            <span>M</span>
        ...
        <div className="flex bg-gray-200 text-xs/6 text-warm-700 lg:flex-auto">
          <div className="hidden w-full lg:grid lg:grid-cols-7 lg:grid-rows-6 lg:gap-px">
            {days.map((day) => (
              <div
                ...
                className="group relative bg-paper-soft px-3 py-2 text-warm-500 data-is-current-month:bg-paper"
              >
                <time
                  dateTime={day.date}
                  className="relative group-not-data-is-current-month:opacity-75 in-data-is-today:flex in-data-is-today:size-6 in-data-is-today:items-center in-data-is-today:justify-center in-data-is-today:rounded-full in-data-is-today:bg-ember in-data-is-today:font-semibold in-data-is-today:text-paper"
                >
                ...
                {day.events.length > 0 ? (
                  <ol className="mt-2">
                    {day.events.slice(0, 2).map((event) => (
                      <li key={event.id}>
                        <a href={event.href} className="group flex">
                          <p className="flex-auto truncate font-medium text-charcoal group-hover:text-ember">
                            {event.name}
                          </p>
                          <time
                            dateTime={event.datetime}
                            className="ml-3 hidden flex-none text-warm-500 group-hover:text-ember xl:block"
                          >
                            {event.time}
                          </time>
                        </a>
                    ...
                    {day.events.length > 2 ? (
                      <li className="text-warm-500">+ {day.events.length - 2} more</li>
                    ) : null}
                  </ol>
                ) : null}
              </div>
          ...
          <div className="isolate grid w-full grid-cols-7 grid-rows-6 gap-px lg:hidden">
            {days.map((day) => (
              <button
                ...
                data-is-current-month={day.isCurrentMonth ? '' : undefined}
                className="group relative flex h-14 flex-col px-3 py-2 not-data-is-current-month:bg-paper-soft not-data-is-selected:not-data-is-current-month:not-data-is-today:text-warm-500 hover:bg-paper-soft focus:z-10 data-is-current-month:bg-paper not-data-is-selected:data-is-current-month:not-data-is-today:text-charcoal data-is-current-month:hover:bg-paper-soft data-is-selected:font-semibold data-is-selected:text-paper data-is-today:font-semibold not-data-is-selected:data-is-today:text-ember"
              >
                <time
                  dateTime={day.date}
                  className="ml-auto group-not-data-is-current-month:opacity-75 in-data-is-selected:flex in-data-is-selected:size-6 in-data-is-selected:items-center in-data-is-selected:justify-center in-data-is-selected:rounded-full in-data-is-selected:not-in-data-is-today:bg-ink in-data-is-selected:in-data-is-today:bg-ember"
                >
                  {day.date.split('-').pop().replace(/^0/, '')}
                </time>
      ...
      <div className="relative px-4 py-10 sm:px-6 lg:hidden">
        <ol className="divide-y divide-gray-100 overflow-hidden rounded-lg bg-paper text-sm shadow-sm outline-1 outline-black/5">
          {events.map((event) => (
            <li
              key={event.id}
              className="group flex p-4 pr-6 focus-within:bg-paper-soft hover:bg-paper-soft"
            >
              <div className="flex-auto">
                <p className="font-semibold text-charcoal">{event.name}</p>
                <time dateTime={event.datetime} className="mt-2 flex items-center text-warm-700">
                  <ClockIcon aria-hidden="true" className="mr-2 size-5 text-warm-500" />
                  {event.time}
                </time>
              </div>
              <a
                href={event.href}
                className="ml-6 flex-none self-center rounded-md bg-paper px-3 py-2 font-semibold text-charcoal opacity-0 shadow-xs ring-1 ring-gray-300 ring-inset group-hover:opacity-100 hover:ring-gray-400 focus:opacity-100"
              >
