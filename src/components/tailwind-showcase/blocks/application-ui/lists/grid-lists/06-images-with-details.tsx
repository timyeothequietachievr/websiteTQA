    ...
    <ul role="list" className="grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 lg:grid-cols-4 xl:gap-x-8">
      {files.map((file) => (
        <li key={file.source} className="relative">
          <div className="group overflow-hidden rounded-lg bg-paper-soft focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-indigo-600">
            <img
              alt=""
              src={file.source}
          ...
          <p className="pointer-events-none mt-2 block truncate text-sm font-medium text-charcoal">
            {file.title}
          </p>
          <p className="pointer-events-none block text-sm font-medium text-warm-500">{file.size}</p>
        </li>
      ))}
    </ul>
  )
