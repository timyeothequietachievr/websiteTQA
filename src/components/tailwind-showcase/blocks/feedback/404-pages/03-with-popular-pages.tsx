    ...
    <div className="bg-paper">
      <main className="mx-auto w-full max-w-7xl px-6 pt-10 pb-16 sm:pb-24 lg:px-8">
        ...
        <div className="mx-auto mt-20 max-w-2xl text-center sm:mt-24">
          ...
          <h1 className="mt-4 text-5xl font-display font-semibold tracking-tight text-balance text-charcoal sm:text-6xl">
        ...
        <div className="mx-auto mt-16 flow-root max-w-lg sm:mt-20">
          ...
          <ul
            role="list"
            className="-mt-6 divide-y divide-charcoal/10/5 border-b border-charcoal/5"
          >
            {links.map((link, linkIdx) => (
              <li key={linkIdx} className="relative flex gap-x-6 py-6">
                <div className="flex size-10 flex-none items-center justify-center rounded-lg shadow-xs outline-1 outline-gray-900/10">
                  <link.icon aria-hidden="true" className="size-6 text-ember" />
                </div>
                <div className="flex-auto">
                  <h3 className="text-sm/6 font-semibold text-charcoal">
                    <a href={link.href}>
                      <span aria-hidden="true" className="absolute inset-0" />
                      {link.name}
                    </a>
                  </h3>
                  <p className="mt-2 text-sm/6 text-warm-500">{link.description}</p>
                </div>
      ...
      <footer className="border-t border-gray-100 py-6 sm:py-10">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-center gap-8 px-6 sm:flex-row lg:px-8">
          <p className="text-sm/7 text-warm-500">&copy; Your Company, Inc. All rights reserved.</p>
          ...
          <div className="flex gap-x-4">
            {social.map((item, itemIdx) => (
              <a
                key={itemIdx}
                href={item.href}
                className="text-warm-500 hover:text-warm-500"
              >
