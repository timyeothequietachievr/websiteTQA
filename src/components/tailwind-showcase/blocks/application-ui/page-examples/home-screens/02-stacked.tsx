...
const statuses = {
  ...
  Withdraw: 'text-warm-500 bg-paper-soft ring-gray-500/10',
              ...
              <div className="order-last flex w-full gap-x-8 text-sm/6 font-semibold sm:order-0 sm:w-auto sm:border-l sm:border-paper-200 sm:pl-6 sm:text-sm/7">
                {secondaryNavigation.map((item) => (
                  <a
                    ...
                    href={item.href}
                    className={
                      item.current ? 'text-ember' : 'text-warm-700'
                    }
                  >
                    {item.name}
                  </a>
              ...
              {stats.map((stat, statIdx) => (
                <div
                  ...
                  className={classNames(
                    ...
                    'flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2 border-t border-charcoal/5 px-4 py-10 sm:px-6 lg:border-t-0 xl:px-8',
                  )}
                >
                  <dt className="text-sm/6 font-medium text-warm-500">{stat.name}</dt>
                  <dd
                    className={classNames(
                  ...
                  <dd className="w-full flex-none text-3xl/10 font-medium tracking-tight text-charcoal">
                    {stat.value}
                  </dd>
                </div>
              ))}
        ...
        <div className="space-y-16 py-16 xl:space-y-20">
          ...
          <div>
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              <h2 className="mx-auto max-w-2xl text-base font-semibold text-charcoal lg:mx-0 lg:max-w-none">
                Recent activity
              </h2>
            </div>
                    ...
                    <tbody>
                      {days.map((day) => (
                        <Fragment key={day.dateTime}>
                          <tr className="text-sm/6 text-charcoal">
                            <th scope="colgroup" colSpan={3} className="relative isolate py-2 font-semibold">
                              <time dateTime={day.dateTime}>{day.date}</time>
                              <div className="absolute inset-y-0 right-full -z-10 w-screen border-b border-paper-200 bg-paper-soft" />
                              <div className="absolute inset-y-0 left-0 -z-10 w-screen border-b border-paper-200 bg-paper-soft" />
                            </th>
                          </tr>
                            ...
                            <tr key={transaction.id}>
                              <td className="relative py-5 pr-6">
                                <div className="flex gap-x-6">
                                  <transaction.icon
                                    aria-hidden="true"
                                    className="hidden h-6 w-5 flex-none text-warm-500 sm:block"
                                  />
                                  <div className="flex-auto">
                                    <div className="flex items-start gap-x-3">
                                      <div className="text-sm/6 font-medium text-charcoal">
                                        {transaction.amount}
                                      </div>
                                      <div
                                        className={classNames(
                                    ...
                                    {transaction.tax ? (
                                      <div className="mt-1 text-xs/5 text-warm-500">
                                        {transaction.tax} tax
                                      </div>
                                    ) : null}
                                  </div>
                              ...
                              <td className="hidden py-5 pr-6 sm:table-cell">
                                <div className="text-sm/6 text-charcoal">{transaction.client}</div>
                                <div className="mt-1 text-xs/5 text-warm-500">
                              ...
                              <td className="py-5 text-right">
                                <div className="flex justify-end">
                                  <a
                                    ...
                                    className="text-sm/6 font-medium text-ember hover:text-indigo-500"
                                  >
                                ...
                                <div className="mt-1 text-xs/5 text-warm-500">
                                  Invoice{' '}
                                  <span className="text-charcoal">#{transaction.invoiceNumber}</span>
                                </div>
                              </td>
                            </tr>
                          ))}
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-2xl lg:mx-0 lg:max-w-none">
              <div className="flex items-center justify-between">
                <h2 className="text-base/7 font-semibold text-charcoal">Recent clients</h2>
                <a
                  href="#"
